...
class Cookie
{
    ...
    public static function init(array $opts = array())
    {
        ...
        if (isset($opts['path']) && (string)$opts['path']) {
            self::$path = (string)$opts['path'];
        }
        if (isset($opts['prefix'])) {
            self::$prefix = (string)$opts['prefix'];
        }
        if (isset($opts['domain']) && (string)$opts['domain']) {
            self::$domain = (string)$opts['domain'];
        } else {
            self::$domain = strtolower($_SERVER['SERVER_NAME']);
            if (preg_match('#^www\.#', strtolower(self::$domain))) {
    ...
    public static function set($name, $value, $expire = self::PERMANENT, $httponly = false)
    {
        ...
        if (is_numeric($expire)) {
            $expire = time() + $expire;
        } else if (is_string($expire) && $expire) {
