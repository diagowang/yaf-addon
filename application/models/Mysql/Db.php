...
class DbModel extends AbstractModel
{
    ...
    {
        try {
            $conf = DbConfig::getDatabaseConfig($this->_databaseKey);
            $databaseConf = array(
                'driver' => 'mysql',
                'port' => 3306,
            ...
            ErrorModel::throwException(CodeConfigModel::CANNOT_CONNECT_DATABASE);
        }
    }
     ...
     * @param $tableName
     * @param array $data
     * @param bool $isReturnLastInsertId
     * @return mixed
     */
    public function insert($tableName, array $data, bool $isReturnLastInsertId = false)
    {
        $result = $this->getDbConnection()->table($tableName)->insert($data);
        if ($isReturnLastInsertId) {
            $result = $this->getDbConnection()->getPdo()->lastInsertId();
        }
        return $result;
    }

    /**
     * 简单的更新数据，仅支持where条件（=，<,>,!=,in,not in,between,not between）
     * @param $tableName
     * @param array $data
     * @param array $where
     * @return int
     */
    public function updateSimple($tableName, array $data, array $where)
    {
    ...
    /**
     * 简单的物理删除数据，仅支持where条件（=，<,>,!=,in,not in,between,not between）
     * @param $tableName
     * @param array $where
     * @return int
     */
    public function deleteSimple($tableName, array $where)
    {
        $builder = $this->getDbConnection()->table($tableName);
        if (empty($where)) {
            return 0;
